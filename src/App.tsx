      ...
      <section className="grid">
        <article className="panel performance">
          ...
          <div className="comparisonMode" role="group" aria-label="情境比較模式">
            <div className={comparisonMode === 'price' ? 'active' : ''}><button onClick={() => setComparisonMode('price')}>成交價</button><label><span>基準情境</span><input aria-label="比較基準成交價" type="number" min="0" step="10000" value={scenario.salePrice || ''} onFocus={() => setComparisonMode('price')} onChange={event => updateScenarioNumber('salePrice', event.target.value === '' ? 0 : Number(event.target.value))}/></label></div>
            <div className={comparisonMode === 'date' ? 'active' : ''}><button onClick={() => setComparisonMode('date')}>成交日期</button><label><span>基準日期</span><input aria-label="比較基準成交日期" type="date" min={activeProperty.mortgageDataDate} value={scenario.saleDate} onFocus={() => setComparisonMode('date')} onChange={event => updateSaleDate(event.target.value)}/></label></div>
          </div>
          <div className="scenarioStepper">
            <div className="baselineStepper"><button type="button" aria-label={comparisonMode === 'price' ? '基準成交價減一格' : '基準出售日減一格'} onClick={() => shiftComparisonBaseline(-1)}>←</button><strong><small>基準情境</small>{comparisonMode === 'price' ? nt(inputs.salePrice) : inputs.saleDate}</strong><button type="button" aria-label={comparisonMode === 'price' ? '基準成交價加一格' : '基準出售日加一格'} onClick={() => shiftComparisonBaseline(1)}>→</button></div>
