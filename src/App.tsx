      ...
      <section className="grid">
        <article className="panel performance">
          ...
          <div className="scenarioStepper">
            <label><span>{comparisonMode === 'price' ? '每格金額' : '每格月數'}</span><input aria-label={comparisonMode === 'price' ? '成交價情境每格金額' : '成交日期情境每格月數'} type="number" min="1" step={comparisonMode === 'price' ? 10_000 : 1} value={comparisonMode === 'price' ? (priceComparisonStep || '') : (dateComparisonStepMonths || '')} onChange={event => comparisonMode === 'price' ? setPriceComparisonStep(event.target.value === '' ? 0 : Number(event.target.value)) : setDateComparisonStepMonths(event.target.value === '' ? 0 : Math.round(Number(event.target.value)))} onBlur={() => comparisonMode === 'price' ? setPriceComparisonStep(current => Math.max(10_000, current || 10_000)) : setDateComparisonStepMonths(current => Math.max(1, current || 1))} /><em>{comparisonMode === 'price' ? '' : '個月'}</em></label>
