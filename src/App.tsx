      ...
      <section className="grid">
        <article className="panel performance">
          ...
          <div className="scenarioStepper">
            <label><span>{comparisonMode === 'price' ? '每格金額' : '每格月數'}</span><input aria-label={comparisonMode === 'price' ? '成交價情境每格金額（萬元）' : '成交日期情境每格月數'} type="number" min="1" step="1" value={comparisonMode === 'price' ? (priceComparisonStep ? priceComparisonStep / 10_000 : '') : (dateComparisonStepMonths || '')} onChange={event => comparisonMode === 'price' ? setPriceComparisonStep(event.target.value === '' ? 0 : Number(event.target.value) * 10_000) : setDateComparisonStepMonths(event.target.value === '' ? 0 : Math.round(Number(event.target.value)))} onBlur={() => comparisonMode === 'price' ? setPriceComparisonStep(current => Math.max(10_000, current || 10_000)) : setDateComparisonStepMonths(current => Math.max(1, current || 1))} /><em>{comparisonMode === 'price' ? '萬元' : '個月'}</em></label>
