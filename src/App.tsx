    ...
    <main>
      ...
      <section className="grid">
        <article className="panel performance">
          ...
          <div className="comparisonMode" role="group" aria-label="情境比較模式">
            <div className={comparisonMode === 'price' ? 'active' : ''}><button onClick={() => setComparisonMode('price')}>成交價</button><label><span>基準情境</span><div className="baselineField">{comparisonMode === 'price' && <button type="button" aria-label="基準成交價減一格" onClick={() => shiftComparisonBaseline(-1)}>←</button>}<input aria-label="比較基準成交價" type="number" min="0" step="10000" value={scenario.salePrice || ''} onFocus={() => setComparisonMode('price')} onChange={event => updateScenarioNumber('salePrice', event.target.value === '' ? 0 : Number(event.target.value))}/>{comparisonMode === 'price' && <button type="button" aria-label="基準成交價加一格" onClick={() => shiftComparisonBaseline(1)}>→</button>}</div></label></div>
            <div className={comparisonMode === 'date' ? 'active' : ''}><button onClick={() => setComparisonMode('date')}>成交日期</button><label><span>基準日期</span><div className="baselineField">{comparisonMode === 'date' && <button type="button" aria-label="基準出售日減一格" onClick={() => shiftComparisonBaseline(-1)}>←</button>}<input aria-label="比較基準成交日期" type="date" min={activeProperty.mortgageDataDate} value={scenario.saleDate} onFocus={() => setComparisonMode('date')} onChange={event => updateSaleDate(event.target.value)}/>{comparisonMode === 'date' && <button type="button" aria-label="基準出售日加一格" onClick={() => shiftComparisonBaseline(1)}>→</button>}</div></label></div>
          </div>
          <div className="scenarioStepper">
            <details className="scenarioStepSettings"><summary><SlidersHorizontal size={13}/>調整每格單位</summary><label><span>{comparisonMode === 'price' ? '每格金額' : '每格月數'}</span><input aria-label={comparisonMode === 'price' ? '成交價情境每格金額（萬元）' : '成交日期情境每格月數'} type="number" min="1" step="1" value={comparisonMode === 'price' ? (priceComparisonStep ? priceComparisonStep / 10_000 : '') : (dateComparisonStepMonths || '')} onChange={event => comparisonMode === 'price' ? setPriceComparisonStep(event.target.value === '' ? 0 : Number(event.target.value) * 10_000) : setDateComparisonStepMonths(event.target.value === '' ? 0 : Math.round(Number(event.target.value)))} onBlur={() => comparisonMode === 'price' ? setPriceComparisonStep(current => Math.max(10_000, current || 10_000)) : setDateComparisonStepMonths(current => Math.max(1, current || 1))} /><em>{comparisonMode === 'price' ? '萬元' : '個月'}</em></label></details>
          </div>
